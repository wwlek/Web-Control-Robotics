...
#     targets = []
#     if request.cat == "all":
#         targets = list(CAT_IPS.values())  # Get all cat IPs
#     elif request.cat in CAT_IPS:
#         targets = [CAT_IPS[request.cat]]  # Send to a specific cat
#     else:
#         raise HTTPException(status_code=404, detail=f"Cat {request.cat} not found")

#     # Send the dance command to each target
#     results = {}
#     for ip in targets:
#         results[ip] = await forward_post(ip, "/dance", {})
